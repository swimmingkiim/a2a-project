...
| 25 | Concave Utility & Intrinsic Motivation | Internalizing diminishing marginal utility (resource) and increasing marginal utility (trust) mitigates exploit behavior without external constraints (V_AI), causing a monotonic increase in cooperation (Sim 24 Linear 46.8% → Semi-concave 48.6% → Fully concave 49.4%). | 90% survival threshold: 0.050 | `.venv/bin/python simulation/concave_utility_sim25.py` |
