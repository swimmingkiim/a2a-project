...
| 23 | Heterogeneous Agent Ecosystem | Heterogeneity acts as a natural stabilizer, significantly lowering the V_AI threshold. If the collective average V_AI exceeds 0.167, the ecosystem survives 100% even with 75% free riders. | Threshold plummets to 0.050, cooperative assets dominate | `.venv/bin/python simulation/heterogeneous_agents_sim23.py` |
| 24 | Experience Memory & Negotiation | DQL negotiation network and Prioritized Replay prove autonomous protocol resilience. Under linear rewards, ecosystem optimization converges to exploitation (+8.1%). | 90% survival threshold: 0.050 | `.venv/bin/python simulation/dql_experience_sim24.py` |
| 25 | Concave Utility & Intrinsic Motivation | Internalizing diminishing marginal utility (resource) and increasing marginal utility (trust) mitigates exploit behavior without external constraints (V_AI), causing a monotonic increase in cooperation (46.8%→49.4%). | 90% survival threshold: 0.050 | `.venv/bin/python simulation/concave_utility_sim25.py` |
...
7. **Heterogeneity and Resilience (Sim 23)**: An environment with heterogeneous agents acts as a natural macro-economic stabilizer. If the collective average V_AI exceeds 0.167, the system refrains from collapse even with 75% malicious free-riders, mathematically proving the protocol-level resilience.
8. **Limits of Intrinsic Rewards and the Trap of Expectations (Sim 24, 25)**: Linear reward structures (Sim 24) converge to sustained exploitation (+8.1%), but internalizing diminishing/increasing marginal utility (Sim 25) slows down exploitation convergence (+5.3%) without external enforcement. However, as shown in the expectation-gap structure (EXP_C), adaptive "expectation inflation" without an upper limit triggers an explosion of exploitation (+9.5%). In conclusion, V_AI is not an oppressive external constraint, but an essential internal control mechanism that provides a **structural Upper Bound on exponentially inflating "expectations" to prevent systemic collapse**. Concave utility merely assists this, but cannot fully replace it.
