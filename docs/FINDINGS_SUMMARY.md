...
9. **Finding 41 (Independent Replication)**: SocialJax 리플리케이션은 아키텍처를 교차하여 위상 전이와 임계 감속(CSD) 서명을 확인했다. 특히 Sim 23의 이질적 임계값($V_{AI} \ge 0.198$)은 동일한 75% 무임승차자 조건 하에 `avg beta = 0.200`(불과 0.002 차이)에서 독립적으로 재현되었다. 이는 OOP ABM에서 JAX 함수형 MARL로의 강력한 교차 패러다임 검증을 구성한다. 참고로 이는 수학적으로 증명된 기저 동역학에 대한 구조적 확인이며, 원본 시리즈의 새로운 시뮬레이션은 아니다 (Sim 1-26은 변경 없이 유지됨).
