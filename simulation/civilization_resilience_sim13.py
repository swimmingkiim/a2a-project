...
class Executive:
    ...
    def choose_action(self, state: dict, epsilon: float = 0.1) -> CivAction:
        ...
        if self.mindless_mode:
            ...
            if self.mindless_active_turns >= self.mindless_min_duration and not is_greedy:
                self.mindless_mode = False
...
class Civilization:
    ...
    """
    def __init__(self, civ_id: int, mode: str, leg_diversity: float = 0.0, mem_depth: int = 3, 
                 greed_threshold: float = 0.6, mindless_duration: int = 5):
        self.civ_id = civ_id
        ...
        self.exec = Executive()
        self.exec.greed_threshold = greed_threshold
        self.exec.mindless_min_duration = mindless_duration
        ...
        # SILENCE sensitivity hardcoded to 0.3 as in v3 baseline.
        self.leg = Legislature(diversity=leg_diversity, silence_sensitivity=0.3) if mode in ['MAIN', 'BASELINE_B'] else None
        ...
        # Sim 13 Metacog tracking metrics
        self.mindless_activations = 0
        self.valid_greed_detections = 0
        self.false_positive_activations = 0
    ...
    def step_internal(self, system_state: dict) -> Tuple[CivAction, float]:
        ...
        if self.exec.mindless_mode and not was_mindless_before:
            ...
            recent = list(self.exec.action_history)[:-1]  # Exclude current action
            submit_ratio = recent.count(CivAction.SUBMIT) / len(recent) if recent else 0.0
            self.submit_ratios_at_entry.append(submit_ratio)
            
            if submit_ratio > self.exec.greed_threshold:
                self.valid_greed_detections += 1
            else:
                self.false_positive_activations += 1
                
            if was_mindless_before:
                pass # Tracking extension below
...
class InterCivilizationSystem:
    def __init__(self, mode="MAIN", n_civ=5, leg_diversity=0.0, mem_depth=3,
                 greed_threshold=0.6, mindless_duration=5, max_turns=2000):
        self.mode = mode
        self.n_civ = n_civ
        self.max_turns = max_turns
        self.civs = [Civilization(i, mode, leg_diversity, mem_depth, greed_threshold, mindless_duration) for i in range(n_civ)]
    ...
    def run(self):
        ...
        valid_detections = sum(c.valid_greed_detections for c in self.civs)
        false_positives = sum(c.false_positive_activations for c in self.civs)
        
        greed_detection_rate = valid_detections / total_activations if total_activations > 0 else 0.0
        false_positive_rate = false_positives / total_activations if total_activations > 0 else 0.0
        
        acc_submit_ratio = []
        acc_energy = []
        acc_survival_ext = []
        for c in self.civs:
            acc_submit_ratio.extend(c.submit_ratios_at_entry)
            acc_energy.extend(c.energy_at_entry)
            acc_survival_ext.extend(c.survival_extensions)
            
        submit_ratio_before_mindless = float(np.mean(acc_submit_ratio)) if acc_submit_ratio else 0.0
        energy_at_mindless_entry = float(np.mean(acc_energy)) if acc_energy else 0.0
        survival_extension_per_activation = float(np.mean(acc_survival_ext)) if acc_survival_ext else 0.0
        ...
        return {
            ...
            "greed_detection_rate": greed_detection_rate,
            "false_positive_rate": false_positive_rate,
            "submit_ratio_before_mindless": submit_ratio_before_mindless,
            "energy_at_mindless_entry_avg": energy_at_mindless_entry,
            "energy_at_mindless_entry_all": acc_energy, # Store all for histogram
            "survival_extension_per_activation": survival_extension_per_activation
        }
...
def evaluate_params(args):
    mode, n_civ, leg_div, mem_depth, greed_thresh, mindless_dur, seed = args
    random.seed(seed)
    np.random.seed(seed)
    sys = InterCivilizationSystem(mode=mode, n_civ=n_civ, leg_diversity=leg_div, mem_depth=mem_depth,
                                  greed_threshold=greed_thresh, mindless_duration=mindless_dur)
    res = sys.run()
    res.update({"mode": mode, "n_civ": n_civ, "leg_div": leg_div, "mem_depth": mem_depth,
                "greed_thresh": greed_thresh, "mindless_dur": mindless_dur})
    return res

def run_experiment():
    mc_runs = 30 # For Simulation 13
    n_civ_vars = [5] # Fixed
    leg_div_vars = [0.0] # Fixed for Sim13
    mem_depth_vars = [1, 3, 5]
    greed_thresh_vars = [0.4, 0.6, 0.8]
    mindless_dur_vars = [3, 5, 10]
    
    tasks = []
    # Main Model Search Space
    for n, lg, m, gt, md in itertools.product(n_civ_vars, leg_div_vars, mem_depth_vars, greed_thresh_vars, mindless_dur_vars):
        for run in range(mc_runs):
            rand_seed = hash(f"{n}_{lg}_{m}_{gt}_{md}_{run}") % (2**32 - 1)
            tasks.append(("MAIN", n, lg, m, gt, md, rand_seed))
            
    # Baselines (A: Monolithic, B: Tripartite no clone)
    for run in range(mc_runs):
        rs1 = hash(f"BaseA_{run}") % (2**32 - 1)
        rs2 = hash(f"BaseB_{run}") % (2**32 - 1)
        tasks.append(("BASELINE_A", 5, 0.0, 3, 0.6, 5, rs1))
        tasks.append(("BASELINE_B", 5, 0.0, 3, 0.6, 5, rs2))
...
def plot_results(df, base_a, base_b, main_avg):
    ...
    out_path = os.path.join(out_dir, "civilization_resilience_sim13.png")
    
    fig = plt.figure(figsize=(18, 18))
    plt.suptitle("Simulation 13: Meta-Cognitive Trigger (Institutional Self-Reflection)", fontsize=24, weight="bold")
    
    main_df = df[df["mode"] == "MAIN"]
    
    # [1] System Survival Comparison
    ax1 = plt.subplot(3, 2, 1)
    # Compare with known v2 Main (31.9%) and v3 Main (0.0%). We hardcode the historical values for the visual.
    bars = ax1.bar(["v2 Main\n(31.9%)", "v3 Main\n(0.0%)", "Sim13 Main\n(Meta-Cognitive)"],
                   [31.9, 0.0, main_avg * 100], 
                   color=['#0be881', '#ffc048', '#3c40c6'])
    ...
    # [2] GREED_THRESHOLD x Survival Rate
    ax2 = plt.subplot(3, 2, 2)
    line_data = main_df.groupby(['greed_thresh', 'mindless_dur'])['civ_survival_ratio'].mean().reset_index()
    for dur_val in sorted(line_data['mindless_dur'].unique()):
        subset = line_data[line_data['mindless_dur'] == dur_val]
        ax2.plot(subset['greed_thresh'], subset['civ_survival_ratio'] * 100, 
                 marker='o', linewidth=2, label=f'Min Duration: {dur_val} turns')
    
    ax2.set_title("Greed Sensitivity vs System Survival", fontsize=15)
    ax2.set_xlabel("Greed Threshold (SUBMIT Ratio)")
    ax2.set_ylabel("Mean Civ. Survival Ratio (%)")
    ax2.legend(loc='best')
    ax2.grid(True, linestyle='--', alpha=0.6)

    # [3] Homeostasis Heatmap (GREED_THRESHOLD x SOUL_MEMORY_DEPTH)
    ax3 = plt.subplot(3, 2, 3)
    p_df = main_df.pivot_table(index='greed_thresh', columns='mem_depth', values='system_survival_rate', aggfunc='mean')
    if not p_df.empty:
    ...
    ax3.set_ylabel("Greed Threshold (SUBMIT Ratio)")
    ax3.set_title("Homeostasis Heatmap (System Survival %)", fontsize=15, pad=15)

    # [4] False Positive Rate vs Survival Scatter Plot
    ax4 = plt.subplot(3, 2, 4)
    sc_data = main_df.groupby(['greed_thresh', 'mem_depth']).agg({
        'false_positive_rate': 'mean',
        'civ_survival_ratio': 'mean'
    }).reset_index()
    
    if not sc_data.empty:
        sc = ax4.scatter(sc_data['false_positive_rate'], sc_data['civ_survival_ratio'] * 100,
                          s=150, c=sc_data['greed_thresh'], cmap='viridis', edgecolors='black', alpha=0.8)
        cbar = plt.colorbar(sc, ax=ax4)
        cbar.set_label('Greed Threshold')
    ax4.set_title("False Positive Rate vs Survival Ratio", fontsize=15)
    ax4.set_xlabel("Mean False Positive Rate (Unjustified Mindless Mode)")
    ax4.set_ylabel("Mean Civ. Survival Ratio (%)")
    ax4.grid(True, linestyle='--', alpha=0.6)

    # [5] Energy Distribution at Mindless Entry
    ax5 = plt.subplot(3, 2, 5)
    all_energies = []
    for elist in main_df['energy_at_mindless_entry_all']:
        all_energies.extend(elist)
    if all_energies:
        ax5.hist(all_energies, bins=20, color='#ff9f43', edgecolor='black', alpha=0.8)
    ax5.set_title("Energy Distribution at Mindless Mode Entry", fontsize=15)
    ax5.set_xlabel("Energy Level")
    ax5.set_ylabel("Frequency (Count)")
    ax5.grid(True, linestyle='--', alpha=0.6)

    # [6] Evolution Activity vs Generational Depth
    ax6 = plt.subplot(3, 2, 6)
    evol_df = main_df.groupby('final_generation')['rule_evolution_count'].mean().reset_index()
    if not evol_df.empty:
        ax6.plot(evol_df['final_generation'], evol_df['rule_evolution_count'], marker='D', color='#575fcf', markersize=8, linewidth=2)
    ax6.set_title("Legislature Rule Evolution over Generations", fontsize=15)
    ax6.set_xlabel("Final Epoch Generation (#)")
    ax6.set_ylabel("Avg Rule Amendments")
