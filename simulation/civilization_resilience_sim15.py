...
class Executive:
    ...
    def __init__(self, w_size: int = 20):
        ...
        self.neighbor_greed_threshold = 0.5
        self.other_awareness_weight = 0.5
    ...
    def _is_ecosystem_greedy(self, neighbor_submit_ratios: List[float]) -> bool:
        if not neighbor_submit_ratios:
            return False
        ecosystem_greed = sum(neighbor_submit_ratios) / len(neighbor_submit_ratios)
    ...
    def choose_action(self, state: dict, epsilon: float = 0.1) -> CivAction:
        ...
        neighbor_ratios = state.get('neighbor_submit_ratios', [])
        
        self_greedy = self._is_being_greedy(current_energy)
        ecosystem_greedy = self._is_ecosystem_greedy(neighbor_ratios)
        ...
        if self.mindless_mode:
            ...
            still_greedy = (
                ...
                (self._is_ecosystem_greedy(neighbor_ratios) and random.random() < self.other_awareness_weight)
            )
...
class Civilization:
    ...
    def step_internal(self, system_state: dict) -> Tuple[CivAction, float]:
        ...
        neighbor_ratios = updated_state.get('neighbor_submit_ratios', [])
        ecosystem_greedy = self.exec._is_ecosystem_greedy(neighbor_ratios)
...
class InterCivilizationSystem:
    def __init__(self, mode="MAIN", n_civ=5, mem_depth=5, energy_gate=2000.0, 
                 neighbor_threshold=0.5, awareness_weight=0.5, max_turns=2000):
        self.mode = mode
        self.n_civ = n_civ
        self.max_turns = max_turns
        self.civs = [Civilization(i, mode, mem_depth, energy_gate, neighbor_threshold, awareness_weight) for i in range(n_civ)]
    ...
    def step(self):
        # 1. Snapshot action history for all civs (Sim 15)
        snapshot_ratios = {}
        for c in self.civs:
            ...
            if len(recent) >= 10:
                ...
                submit_ratio = 0.0
            snapshot_ratios[c.civ_id] = submit_ratio if not c.is_dead else 0.0
        ...
        for civ in self.civs:
            if not civ.is_dead:
                # Inject Neighbor Submit Ratios (Sim 15)
                neighbor_ratios = [ratio for neighbor_id, ratio in snapshot_ratios.items() if neighbor_id != civ.civ_id]
                system_state_civ = system_state.copy()
                system_state_civ['neighbor_submit_ratios'] = neighbor_ratios
    ...
    def run(self):
        ...
        # Sim 15 specific aggregation
        total_mindless_activations = sum(c.mindless_activations for c in self.civs)
        total_ecosystem_triggers = sum(c.ecosystem_triggers for c in self.civs)
        total_preemptive_restraints = sum(c.preemptive_restraints for c in self.civs)
        total_free_rider_turns = sum(c.free_rider_turns for c in self.civs)
        total_eco_greedy_turns = sum(c.ecosystem_greedy_turns for c in self.civs)
        
        ecosystem_trigger_rate = (total_ecosystem_triggers / total_mindless_activations) if total_mindless_activations > 0 else 0.0
        preemptive_restraint_rate = (total_preemptive_restraints / total_mindless_activations) if total_mindless_activations > 0 else 0.0
        free_rider_detection = (total_free_rider_turns / total_eco_greedy_turns) if total_eco_greedy_turns > 0 else 0.0
        ...
        return {
            ...
            "ecosystem_trigger_rate": ecosystem_trigger_rate,
            "preemptive_restraint_rate": preemptive_restraint_rate,
            "free_rider_detection": free_rider_detection,
        }
...
def evaluate_params(args):
    mode, n_civ, mem_depth, energy_gate, nb_thresh, aw_weight, seed = args
    random.seed(seed)
    np.random.seed(seed)
    sys = InterCivilizationSystem(mode=mode, n_civ=n_civ, mem_depth=mem_depth, energy_gate=energy_gate,
                                  neighbor_threshold=nb_thresh, awareness_weight=aw_weight)
    res = sys.run()
...
def run_experiment():
    mc_runs = 30 # For Simulation 15
    n_civ_vars = [5] # Fixed
    mem_depth_vars = [5] # Fixed
    energy_gate_vars = [500, 2000]
    nb_threshold_vars = [0.4, 0.6, 0.8]
    awareness_weight_vars = [0.3, 0.6, 0.9]
    
    tasks = []
    # Main Model Search Space
    for n, m, eg, nb, aw in itertools.product(n_civ_vars, mem_depth_vars, energy_gate_vars, nb_threshold_vars, awareness_weight_vars):
        for run in range(mc_runs):
            rand_seed = hash(f"{n}_{m}_{eg}_{nb}_{aw}_{run}") % (2**32 - 1)
            tasks.append(("MAIN", n, m, eg, nb, aw, rand_seed))
            
    # Baselines (A: Monolithic, B: Tripartite no clone)
    for run in range(mc_runs):
        rs1 = hash(f"BaseA_{run}") % (2**32 - 1)
        rs2 = hash(f"BaseB_{run}") % (2**32 - 1)
        tasks.append(("BASELINE_A", 5, 5, 2000.0, 0.5, 0.5, rs1))
        tasks.append(("BASELINE_B", 5, 5, 2000.0, 0.5, 0.5, rs2))
    ...
    plot_results(df, main_sr)

def plot_results(df, main_avg):
    PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    out_dir = os.path.join(PROJECT_ROOT, "docs", "assets")
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, "civilization_resilience_sim15.png")
    
    fig = plt.figure(figsize=(18, 12))
    plt.suptitle("Simulation 15: Other-Aware Meta-Cognition", fontsize=24, weight="bold")
    
    main_df = df[df["mode"] == "MAIN"]
    
    # [1] System Survival Progress
    ax1 = plt.subplot(2, 3, 1)
    bars = ax1.bar(["Sim13", "Sim14", "Sim15\n(Other-Aware)"],
                   [11.6, 38.1, main_avg * 100], 
                   color=['#3c40c6', '#ef5777', '#0be881'])
    ax1.set_title("System Survival Progress", fontsize=15)
    ax1.set_ylabel("System Survival Rate (%)", fontsize=12)
    ax1.axhline(y=31.9, color='r', linestyle='--', label="v2 Baseline (31.9%)")
    ax1.set_ylim(0, 105)
    for bar in bars:
        yval = bar.get_height()
    ...
    # [2] OTHER_AWARENESS_WEIGHT x Survival Line Chart
    ax2 = plt.subplot(2, 3, 2)
    line_data = main_df.groupby(['awareness_weight', 'neighbor_threshold'])['system_survival_rate'].mean().reset_index()
    for nb_val in sorted(line_data['neighbor_threshold'].unique()):
        subset = line_data[line_data['neighbor_threshold'] == nb_val]
        ax2.plot(subset['awareness_weight'], subset['system_survival_rate'] * 100, 
                 marker='o', linewidth=2, label=f'Neighbor Thresh: {nb_val}')
    
    ax2.set_title("Other Awareness vs Survival", fontsize=15)
    ax2.set_xlabel("Other Awareness Weight")
    ax2.set_ylabel("System Survival Rate (%)")
    ax2.legend(loc='best')
    ax2.grid(True, linestyle='--', alpha=0.6)

    # [3] Collapse Cause Shift (Sim14 vs Sim15)
    ax3 = plt.subplot(2, 3, 3)
    avg_starve = main_df['starvation_rate'].mean() * 100
    avg_os = main_df['over_submission_rate'].mean() * 100
    avg_ce = main_df['conflict_rate'].mean() * 100
    
    # Sim 14 ratios (estimated based on user prompt: OS=1587, ST=1069, CE=773 -> Total=3429)
    s14_total = 1587 + 1069 + 773
    s14_os, s14_st, s14_ce = (1587/s14_total)*100, (1069/s14_total)*100, (773/s14_total)*100
    
    bar_width = 0.35
    x = np.arange(3)
    ax3.bar(x - bar_width/2, [s14_os, s14_st, s14_ce], width=bar_width, label='Sim 14', color='#eb3b5a')
    ax3.bar(x + bar_width/2, [avg_os, avg_starve, avg_ce], width=bar_width, label='Sim 15 (Other-Aware)', color='#0fb9b1')
    ...
    # [4] NEIGHBOR_GREED_THRESHOLD x OTHER_AWARENESS_WEIGHT Heatmap
    ax4 = plt.subplot(2, 3, 4)
    p_df = main_df.pivot_table(index='neighbor_threshold', columns='awareness_weight', values='system_survival_rate', aggfunc='mean')
    if not p_df.empty:
        cax = ax4.matshow(p_df, cmap='YlGnBu')
        plt.colorbar(cax, ax=ax4, fraction=0.046, pad=0.04)
        ax4.set_xticks(range(len(p_df.columns)))
        ax4.set_xticklabels(p_df.columns)
        ax4.set_yticks(range(len(p_df.index)))
        ax4.set_yticklabels(p_df.index)
        for (i, j), z in np.ndenumerate(p_df):
            ax4.text(j, i, f"{z*100:.1f}%", ha='center', va='center', fontweight='bold', color='white' if z > 0.5 else 'black')
    ax4.set_xlabel("Other Awareness Weight")
    ax4.set_ylabel("Neighbor Greed Threshold")
    ax4.set_title("Homeostasis Heatmap (Survival %)", fontsize=15, pad=15)

    # [5] Preemptive Restraint Rate vs Survival (Scatter)
    ax5 = plt.subplot(2, 3, 5)
    ax5.scatter(main_df['preemptive_restraint_rate'], main_df['civ_survival_ratio'] * 100, alpha=0.6, color='#8e44ad')
    if not main_df['preemptive_restraint_rate'].isna().all() and len(main_df) > 1:
        z = np.polyfit(main_df['preemptive_restraint_rate'].fillna(0), main_df['civ_survival_ratio'].fillna(0) * 100, 1)
        p = np.poly1d(z)
        ax5.plot(main_df['preemptive_restraint_rate'], p(main_df['preemptive_restraint_rate']), "r--", alpha=0.8)
    ax5.set_title("Preemptive Restraint vs Survival", fontsize=15)
    ax5.set_xlabel("Preemptive Restraint Rate")
    ax5.set_ylabel("Civ Survival Ratio (%)")
    ax5.grid(True, linestyle='--', alpha=0.6)

    # [6] Free Rider Detection vs Conflict Rate (Scatter)
    ax6 = plt.subplot(2, 3, 6)
    ax6.scatter(main_df['free_rider_detection'], main_df['conflict_rate'] * 100, alpha=0.6, color='#d35400')
    if not main_df['free_rider_detection'].isna().all() and len(main_df) > 1:
        z = np.polyfit(main_df['free_rider_detection'].fillna(0), main_df['conflict_rate'].fillna(0) * 100, 1)
        p = np.poly1d(z)
        ax6.plot(main_df['free_rider_detection'], p(main_df['free_rider_detection']), "r--", alpha=0.8)
    ax6.set_title("Free Rider vs Conflict Rate", fontsize=15)
    ax6.set_xlabel("Free Rider Occurrence Rate")
    ax6.set_ylabel("Conflict Collapse Rate (%)")
