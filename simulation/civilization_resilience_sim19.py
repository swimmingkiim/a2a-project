...
class InterCivilizationSystem:
    ...
    def step(self):
        ...
        living_cost = 6.0 + (3.0 if self.pandemic_active else 0.0)
        for civ in self.civs:
...
def plot_results(df):
    ...
    ax1.axhline(y=39.3, color='k', linestyle='--', label='Sim18 Baseline')
    ...
    for scen in ['NONE', 'CASCADE', 'PEAK', 'EARLY']:
    ...
    ax3.set_title("Population Density Trajectory (Assist=True)", fontsize=14)
    ax3.set_xlabel("Turn")
    ...
    ax4.text(0, 0.65, "Pre-Shock (Turn 590)", ha='center', fontweight='bold')
    ax4.text(1.2, 0.65, "Post-Shock Final", ha='center', fontweight='bold')
    ax4.axis('equal')
