...
async function runTests() {
    ...
    // Test 1: Success
    {
        ...
        const req = {
            headers: { authorization: `Bearer ${generateMockVC('did:web:test_project', '0xVALID_WALLET')}` }
        };
        const res = mockRes();

        await handleGrantRequest(req, res, { db: mockDb, vcHandler: mockVcHandler, grantService: mockGrantService });
    ...
    // Test 2: Invalid DID (Not Registered)
    {
        ...
        const req = {
            headers: { authorization: `Bearer ${generateMockVC('did:web:unregistered', '0xVALID_WALLET')}` }
        };
        const res = mockRes();
    ...
    // Test 3: Already Claimed DID
    {
        ...
        const req = {
            headers: { authorization: `Bearer ${generateMockVC('did:web:already_claimed', '0xNEW_WALLET')}` }
        };
        const res = mockRes();
