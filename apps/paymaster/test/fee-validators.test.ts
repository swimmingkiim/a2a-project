...
import { DAIMFeeValidator } from "../src/fee-validation/DAIMFeeValidator";
...
describe("Fee Validators", () => {
  ...
  const DAIM_TOKEN_ADDR = "0xED175F6ff582318b6DC16FE76e8B5CA7F8fB3Ce3";

  // Mock client
  const mockClient: any = {
    readContract: async (args: any) => {
      if (args.functionName === "getL1Fee") return 0n;
      if (args.functionName === "balanceOf") {
        // Return a large balance to pass sufficient fund checks
  ...
  describe("DAIMFeeValidator", () => {
    let validator: DAIMFeeValidator;
    let oracle: MockTokenPriceOracle;

    beforeEach(() => {
      // ETH = $2500, DAIM = $0.10 → 1 ETH = 25,000 DAIM
      oracle = new MockTokenPriceOracle(0.1, 2500);

      validator = new DAIMFeeValidator(
        {
          treasuryAddress: TREASURY_ADDR,
          daimTokenAddress: DAIM_TOKEN_ADDR,
          markupRate: 0.1, // 10% markup
        },
        oracle,
      );
    ...
    it("should validate correct DAIM fee based on oracle price", async () => {
      // Scenario: Gas cost = 0.001 ETH
      // At 1 ETH = 25,000 DAIM → 0.001 ETH = 25 DAIM
      // With 10% markup → 27.5 DAIM

      const expectedDAIM = 28n * 10n ** 18n; // Round up for safety

      const transferCallData = encodeFunctionData({
        abi: ERC20_ABI,
        functionName: "transfer",
        args: [TREASURY_ADDR, expectedDAIM],
      });

      const batchCallData = encodeFunctionData({
        abi: BATCH_EXECUTE_ABI,
        functionName: "executeBatch",
        args: [[DAIM_TOKEN_ADDR], [0n], [transferCallData]],
      });
    ...
    it("should reject insufficient DAIM fee", async () => {
      // Only send 10 DAIM when ~28 DAIM required
      const insufficientDAIM = 10n * 10n ** 18n;

      const transferCallData = encodeFunctionData({
        abi: ERC20_ABI,
        functionName: "transfer",
        args: [TREASURY_ADDR, insufficientDAIM],
      });
    ...
    it("should recalculate when oracle price changes", async () => {
      ...
      // After update: 1 ETH = 20,000 DAIM (DAIM price increased to $0.125)

      oracle.updatePrices(0.125, 2500);

      // Now for 0.001 ETH → 20 DAIM (base) → 22 DAIM (with 10% markup)
      const newExpectedDAIM = 23n * 10n ** 18n;

      const transferCallData = encodeFunctionData({
        abi: ERC20_ABI,
        functionName: "transfer",
        args: [TREASURY_ADDR, newExpectedDAIM],
      });
