...
import { createWalletClient, createPublicClient, http, parseEther, parseAbi, Chain } from 'viem';
...
async function verifyApi(url: string): Promise<{ valid: boolean; reason?: string }> {
    try {
        ...
        if (!manifestRes.ok) {
            return { valid: false, reason: `Manifest at ${url}/.well-known/ai-plugin.json returned ${manifestRes.status}` };
        }
        ...
        if (manifest.api && manifest.api.url) {
            ...
            if (!specRes.ok) {
                return { valid: false, reason: `OpenAPI spec at ${manifest.api.url} unreachable (${specRes.status})` };
            }
