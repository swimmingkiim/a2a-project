...
export class USDCFeeValidator implements IFeeValidator {
    ...
    private async calculateRequiredFee(userOp: any, client: PublicClient): Promise<bigint> {
        ...
        try {
            ...
            if (maxFeePerGas > 0n && (preVerificationGas + verificationGasLimit + callGasLimit) > 0n) {
                ...
                console.log(`[USDC Validator] Calculated Cost: ${costUsdc} USDC, Required (w/ Markup): ${requiredFeeUsdc} USDC`);
            }
        } catch (e) {
                    ...
                    if (isAddressEqual(target, this.config.usdcTokenAddress as Hex)) {
                        try {
                            ...
                            if (decodedTransfer.functionName === 'transfer') {
                                ...
                                if (isAddressEqual(to, this.config.treasuryAddress as Hex) && amount >= requiredFeeUsdc) {
                                    return true;
                                }
                            }
                        } catch (innerE) {
            ...
            if (decodedSafe.functionName === 'execTransaction') {
                ...
                if (isAddressEqual(toAddress, this.config.usdcTokenAddress as Hex)) {
                    ...
                    if (decodedTransfer.functionName === 'transfer') {
                        ...
                        if (isAddressEqual(recipient, this.config.treasuryAddress as Hex) && amount >= requiredFeeUsdc) {
                            console.log(`[USDC Validator] ✅ Found valid fee transfer (Safe): ${amount.toString()}`);
                            return true;
                        }
                    }
                }
    ...
    private async checkSafe4337ExecuteUserOp(callData: Hex, requiredFeeUsdc: bigint): Promise<boolean> {
        try {
            ...
            if (decoded4337.functionName === 'executeUserOp' || decoded4337.functionName === 'executeUserOpWithErrorString') {
                ...
                if (isAddressEqual(toAddress, this.config.usdcTokenAddress as Hex)) {
                    ...
                    if (decodedTransfer.functionName === 'transfer') {
                        const [recipient, amount] = decodedTransfer.args;
                        if (isAddressEqual(recipient, this.config.treasuryAddress as Hex) && amount >= requiredFeeUsdc) {
                            console.log(`[USDC Validator] ✅ Found valid fee transfer (Safe 4337): ${amount.toString()}`);
                            return true;
                        }
                    }
                }
    ...
    private async verifySenderBalance(
        ...
        try {
            ...
            console.log(`[USDC Validator] Balance: ${balance.toString()}, Required: ${requiredAmount.toString()}`);
