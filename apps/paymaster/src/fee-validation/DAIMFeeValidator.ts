...
export class DAIMFeeValidator implements IFeeValidator {
  ...
  private async calculateRequiredFee(userOp: any, client: PublicClient): Promise<bigint> {
    try {
      ...
      console.log(
        `[DAIM Validator] Base DAIM: ${requiredDAIM}, With Markup: ${requiredDAIMWithMarkup}`,
      );
          ...
          if (isAddressEqual(target, this.config.daimTokenAddress as Hex)) {
            ...
            if (decodedTransfer.functionName === "transfer") {
              ...
              if (
                ...
                amount >= requiredFeeDAIM
              ) {
                console.log(
                  `[DAIM Validator] ✅ Found valid DAIM fee transfer: ${amount.toString()} to ${to}`,
                );
                return true;
              }
            }
        ...
        if (isAddressEqual(target, this.config.daimTokenAddress as Hex)) {
          ...
          if (decodedTransfer.functionName === "transfer") {
            ...
            if (
              ...
              console.log(
                `[DAIM Validator] ✅ Found valid DAIM fee transfer: ${amount.toString()}`,
              );
              return true;
            }
          }
        ...
        if (isAddressEqual(toAddress, this.config.daimTokenAddress as Hex)) {
          ...
          if (decodedTransfer.functionName === "transfer") {
            ...
            if (
              ...
              console.log(
                `[DAIM Validator] ✅ Found valid DAIM fee transfer (Safe): ${amount.toString()}`,
              );
              return true;
            }
          }
        ...
        if (isAddressEqual(toAddress, this.config.daimTokenAddress as Hex)) {
          ...
          if (decodedTransfer.functionName === "transfer") {
            ...
            if (
              ...
              console.log(
                `[DAIM Validator] ✅ Found valid DAIM fee transfer (Safe 4337): ${amount.toString()}`,
              );
              return true;
            }
          }
  ...
  private async verifySenderBalance(
    ...
    try {
      ...
      console.log(
        `[DAIM Validator] Balance: ${balance.toString()}, Required: ${requiredAmount.toString()}`,
      );
